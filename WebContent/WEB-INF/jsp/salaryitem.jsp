...
<body class="childrenBody">
	<div class="layui-row">
		<table class="layui-hide layui-col-xs10" id="table_menu"
			lay-filter="menu"></table>
	</div>
								...
								layer
										.open({
											...
											success : function(layero, index) {
												...
												console.info(data);
												var menuId = body
														.find('input[name=menuId]');
												menuId.val(data.menuId);
												var menuName = body
														.find('input[name=meuName]');
												menuName.val(data.meuName);
												var menuLink = body
														.find('input[name=menuLink]');
												menuLink.val(data.menuLink);
												var ifrme = body.context
												...
												for (var i = 0; i < dds.length; i++) {
													if (dds[i]
															.getAttribute('lay-value') == data.menuFather) {
														dds[i].click();
														return;
													}
												}
												var munuCkeckBox = body
														.find('div.layui-form-checkbox');
												...
												if (data.menuImg != undefined) {
													img
															.append('<img src="../resources/'+data.menuImg+'" style="height:30px;weight:30px">');
												}
											},
											yes : function(index) {
												alert("yes");
												var form = layer.getChildFrame(
														'#addForm', index);
												var submit = form
														.find('#submit');
												submit.click();
